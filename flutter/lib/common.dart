...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // jingyue://<connect-id>
    // jingyue://<connect-id>/r
    // jingyue://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing JingyueControl...");
...
const kMacOSPermChannel = MethodChannel("org.jingyue.jingyue_control/host");
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://jingyue.rqrt2122.cn'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
...
bool whitelistNotEmpty() {
  // https://jingyue.rqrt2122.cn/docs/en/self-host/client-configuration/advanced-settings/#whitelist
