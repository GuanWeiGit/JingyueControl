...
{
  HINSTANCE hInstance = LoadLibraryA("libjingyue_control.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libjingyue_control.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN jingyue_control_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "jingyue_control_core_main_args");
  if (!jingyue_control_core_main)
  {
    std::cout << "Failed to get jingyue_control_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  int args_len = 0;
  char** c_args = jingyue_control_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "JingyueControl [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"JingyueControl";
  FUNC_RUSTDESK_GET_APP_NAME get_jingyue_control_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_jingyue_control_app_name");
  if (get_jingyue_control_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_jingyue_control_app_name(app_name_buffer, 512) == 0) {
